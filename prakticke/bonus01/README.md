...
Vaše riešenie by malo v metóde `prove`:

1. najskôr zakódovať **iba teóriu** a pustiť na ňu SAT solver, aby ste sa
   ubezpečili, že vaša teória má model (riešenie) a **našli možných vrahov**;
2. potom by malo **pre každého možného vraha** znovu zakódovať teóriu spolu
   s tvrdením, ktoré chcete **dokázať**, znovu zavolať SAT solver
   a skontrolovať, či dá očakávaný výstup.

Metóda `prove` má vrátiť zoznam všetkých osôb, o ktorých sa **podarilo
dokázať**, že sú vrahom.

**Testovač kontroluje, či máte dobre implementovanú metódu `writeTheory`,
ale nie, či máte správne dokazovanie v metóde `prove`.** Overí však, či ste
správne určili vraha/-ov.
