...
public class TableauTest {
    ...
    public void testSignedForm(
        ...
        t.testCase("signedType/signedSubfs " + f.toString(), s -> {

            SignedFormula.Type typeT = f.signedType(true);
            SignedFormula.Type typeF = f.signedType(false);
            List<SignedFormula> sfTs = f.signedSubfs(true);
            List<SignedFormula> sfFs = f.signedSubfs(false);
            ...
            t.compare(sfsT, expSfsT,
                String.format("%s.signedSubfs(true)", f.toString()));
            t.compare(sfsF, expSfsF,
                String.format("%s.signedSubfs(false)", f.toString()));
        });
    }
    ...
    void testTableauStructure(
        ...
        if (src == null) {
            ...
            if (!src.sf().subfs().contains(node.sf()))
                throw new BadTableauException(t, String.format(
                    "Node (%d) doesn't contain a subformula of it's source (%d)."
                    + "\n [ node.sf: %s , source.sf: %s ]",
                    node.number(), src.number(), node.sf().toString(), src.sf().subfs().toString()));
            ...
            if (type == Beta) {
                if (parent.children().size() != src.sf().subfs().size())
                    throw new BadTableauException(t, String.format(
                        "Node (%d) should have %d siblings because it's a result of Beta rule for (%d).",
                        node.number(), src.sf().subfs().size(), src.number()));
                    ...
                    if (type == Alpha) {
                        ...
                        if (!have)
                            throw new BadTableauException(t, String.format(
                                ...
                                node.number(), nd.number(),Arrays.asList(nd.sf().subfs())));
                    }
                }
            }
        } else {
