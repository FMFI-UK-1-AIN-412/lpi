...
Aby ste neboli zahltení chybami, môžete zastaviť testovanie po prvej chybe
prepnutím statického atribútu [`stopOnFirstFailure`](pu04-java/CnfTest.java#L57) v triede `CnfTest` na `true`.
...
Ak vaša implementácia nepotrebuje konverziu do NNF, môžete vypnúť jej testovanie
nastavením statického atribútu [`skipNnf`](pu04-java/CnfTest.java#L56) v triede `CnfTest` na `true`.
